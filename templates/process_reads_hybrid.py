...
if __file__.endswith(".command.sh"):
    SAMPLE_ID = '$sample_id'
    FASTQ = '$fastq_short'.split()
    ONT = '$fastq_log'.split()
    logger.debug("Running {} with parameters:".format(
        os.path.basename(__file__)))
    logger.debug("SAMPLE_ID: {}".format(SAMPLE_ID))
    logger.debug("FASTQ: {}".format(FASTQ))
    logger.debug("ONT: {}".format(ONT))


def main(sample_id, fastq, ont):

    # get total number of reads - short
    n_reads_short = (sum(1 for line in gzip.open(fastq[0], 'rb'))/4)  # To get read pairs seeing only one file is enough

...
def main(sample_id, assembler, assembly):
            ...
            "tableRow": [{
                ...
                "data": [
                    ...
                    {"header": "contigs>1000bp (%)",
                     "value": f'{len(contigs_over_1000bp)} ({(len(contigs_over_1000bp)/len(contigs))*100:.2f}%)',
                     "table": "assembly_global_stats",
                     "sample": sample_id},
                    {"header": "Basepairs in contigs>1000bp (%)",
                     "value": f'{sum(contigs_over_1000bp)} ({(sum(contigs_over_1000bp)/sum(contigs))*100:.2f}%)',
                     "table": "assembly_global_stats",
                     "sample": sample_id},
    ...
    with open(sample_id + '_' + assembler + "_global_assembly_stats.csv", "w") as cvs_file:
